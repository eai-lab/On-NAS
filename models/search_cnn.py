...
class SearchCNN(nn.Module):
    ...
    def __init__(
        ...
        if not reduction_layers:
            self.reduction_layers = [n_layers // 3, (2 * n_layers) // 3]
        ...
        for i in range(n_layers):
            ...
            if i in self.reduction_layers:
                ...
                reduction = False
            cell = SearchCell(
                n_nodes, C_pp, C_p, C_cur, reduction_p, reduction, PRIMITIVES,config
            )
        ...
        self.wocelllist = nn.ModuleList([self.stem,self.gap,self.linear])
    ...
    def forward(
        ...
        for idx, cell in enumerate(self.cells):
            ...
            # print("cell",f"output {idx} allocated peak : {torch.cuda.memory_stats(0)['allocated_bytes.all.peak']/(1024**2): .2f}MB")

            # print(f"internal phase : {self.config.cell_phase}, cellidx : {self.config.cell_idx}")
            prohibited_list = self.config.prohibited_list
            if self.config.dataset not in prohibited_list and self.config.light_exp == 1:
                if self.config.cell_phase == self.config.cell_idx and self.config.cell_phase != self.config.layers -1:
                    return s1
                
        if self.config.layers - 1 == self.config.cell_phase or self.config.dataset in prohibited_list or self.config.light_exp == 0:
            out = self.gap(s1)
...
class SearchCell(nn.Module):
    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction, PRIMITIVES,config,previous_alphas=None):
        """
        Args:
        ...
        # self.preproc0_reduc = ops.FactorizedReduce(C_pp, C, affine=False)
        
        # self.preproc0 = ops.StdConv(C_pp, C, 1, 1, 0, affine=False)
        # self.preproc1 = ops.StdConv2(C_p, C, 1, 1, 0, affine=False)
        
        self.preproc0_post_reduc = ops.FactorizedReduce(C_pp, C, affine=False)
        
        self.preproc0 = ops.StdConv(C_pp, C, 1, 1, 0, affine=False)
        self.preproc1 = ops.StdConv(C_p, C, 1, 1, 0, affine=False)
        # generate dag
        
        if int(self.config.stackup_phase) == 1:
            if reduction:
                gene = PRIMITIVES.reduce
                self.concat = PRIMITIVES.reduce_concat
            else:
                gene = PRIMITIVES.normal
                self.concat = PRIMITIVES.normal_concat
            self.dag = nn.ModuleList()
            for edges in gene:
                row = nn.ModuleList()
                for op_name, s_idx in edges:
                    stride = 2 if reduction and s_idx <2 else 1
                    op = ops.OPS[op_name](C, stride, True)
            ...
            for i in range(self.n_nodes):
                ...
                for j in range(2 + i):  # include 2 input nodes
                    ...
                    op = ops.MixedOp(C, stride, PRIMITIVES,config)
    ...
    def alphas(self):
        for n,p in self.previous_alphas:
            yield p
    ...
    def cell_tensor_hook(self,grad):
        if self.alpha_switch == 1:
            self.config.alpha_previous_grad = grad
        else:
            self.config.previous_grad = grad

        return grad
    
    def cell_tensor_hook2(self,grad):
        if self.alpha_switch == 1:
            self.config.alpha_pprevious_grad = grad
        else:
            self.config.pprevious_grad = grad
    ...
    def forward(
        ...
        if not self.config.naivenaive and self.config.cell_phase == self.config.cell_idx and self.config.eval_switch!=1 and s1.requires_grad == True:
            s1.register_hook(self.cell_tensor_hook)
            if self.config.residual_flag == 1:
                if self.config.cell_phase == 2 or self.config.cell_phase == 3:
                    s0.register_hook(self.cell_tensor_hook2)


        if self.reduction_p:
            s0 = self.preproc0_post_reduc(s0)
        else:    
            s0 = self.preproc0(s0)
        ...
        s1 = self.preproc1(s1)
        a_metrics = self.config.alpha_sample_metrics
        ...
        if w_input is not None:  # hierarchical alphas #####deprecated
            ...
            for edges, w_node_ops, w_node_in in zip(self.dag, w_dag, w_input):
                s_cur = 0.0 #this made 0.0 on my means lol 
            ...
            if a_metrics is not None:
                for edges, w_node_ops, w_node_pw, a_metric in zip(self.dag, w_dag, w_pw,a_metrics):
                    ...
                    for i, (state_in, w_ops,a_metric_line) in enumerate(zip(states, w_node_ops,a_metric)):
                        input_cur = edges[i](
                            state_in, w_ops, a_metric = a_metric_line,alpha_prune_threshold=alpha_prune_threshold 
                        )
                    ...
                    for i, sum_pw in enumerate(
                        ...
                        if self.config.beta_sampling == 1 and self.config.naivenaive != 1:
                            if len(pairwise_inputs)>2 :
                                if i != dice:
                                    with torch.no_grad():
                                    ...
                                    with torch.enable_grad():
                                        if w_node_pw[dice] > alpha_prune_threshold:
                                            s_cur = s_cur + sum_pw * w_node_pw[dice]
            ...
            # iterate over nodes in cell
                for edges, w_node_ops, w_node_pw in zip(self.dag, w_dag, w_pw):
                    ...
                    #####################################Making sum of edge. input_cur is the summation of edges at Points.
                    # iterate over inputs per node
                    for i, (state_in, w_ops) in enumerate(zip(states, w_node_ops)):
                        ...
                        unariy_inputs.append(input_cur)
                    ###################################Therefore, we can say this is the state. uniary inputs are the 'original states'
                    # build pairwise sums
            ...
            if self.config.stackup_phase == 1:
