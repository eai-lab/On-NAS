...
class FactorizedReduce(nn.Module):
    ...
    def __init__(self, C_in, C_out, affine=True):
        ...
        self.relu = nn.ReLU()
        self.switch = 0
        self.conv1 = nn.Conv2d(C_in, C_out // 2 + 1 , 1, stride=2, padding=0, bias=False)
...
class FactorizedReduce_pre(nn.Module):
    """
    Reduce feature map size by factorized pointwise(stride=2).
    """
...
class MixedOp(nn.Module):
    ...
    def __init__(self, C, stride, PRIMITIVES):
        ...
        for primitive in PRIMITIVES:
            op = OPS[primitive](C // 4, stride, affine=False)
    ...
    def forward(self, x, weights, alpha_prune_threshold=0.0):
        ...
        x_piece_1 = x[:, :channel_dim // 4, :, :]
        x_piece_2 = x[:, channel_dim // 4:, :, :]
        
        result_1 = sum( w * op(x_piece_1) for w, op in zip(weights, self._ops) if w > alpha_prune_threshold)
        
        if result_1.shape[2] == x.shape[2] :
            
            result = torch.cat([result_1, x_piece_2], dim=1)
        else:
            result = torch.cat([result_1, self.mp(x_piece_2)], dim=1)
        result = channel_shuffle(result, 4)
        return result
