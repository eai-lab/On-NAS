...
class SearchCNNControllerPC(nn.Module):
...
class SearchCNNPC(nn.Module):
    ...
    def forward(
        ...
        for cell in self.cells:
            ...
            weights_pc = weights_pc_reduce if cell.reduction else weights_pc_normal
            s0, s1 = s1, cell(
                s0,
                s1,
                weights,
...
class SearchCellPC(nn.Module):
    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction, PRIMITIVES):
        ...
        if reduction_p:
            self.preproc0 = ops_7c.FactorizedReduce_pre(C_pp, C, affine=False)
        else:
            self.preproc0 = ops_7c.StdConv(C_pp, C, 1, 1, 0, affine=False)
        self.preproc1 = ops_7c.StdConv(C_p, C, 1, 1, 0, affine=False)
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2 + i):  # include 2 input nodes
                ...
                op = ops_7c.MixedOp(C, stride, PRIMITIVES)
                self.dag[i].append(op)

    def forward(
        self, s0, s1, w_dag, w_input=None, w_pw=None, w_pc=None, alpha_prune_threshold=0.0
    ):
        ...
        if w_input is not None:  # hierarchical alphas #####deprecated
            ...
            for edges, w_node_ops, w_node_in in zip(self.dag, w_dag, w_input):
                ...
                states.append(s_cur)
        elif w_pc is not None:
            
            # iterate over nodes in cell
            for edges, w_node_ops, w_node_in in zip(self.dag, w_dag, w_pc):
                s_cur = 0.0
                # iterate over inputs of node
                for i, (state_in, w_ops, w_edge) in enumerate(
                    zip(states, w_node_ops, w_node_in)
                ):
                    if w_edge > 0:
                        s_cur = s_cur + w_edge * edges[i](state_in, w_ops)
            ...
            # We should use w_pw, but still have to accesible to the w_pc. 
            # So in this case, our work here is fine i guess. 
            # I initialized it. 
            # I normalized it every single time.
            # Yet it is outside of Computational graph, 
